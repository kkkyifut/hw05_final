...
@login_required
def post_comment(request, username, post_id) -> HttpResponse:
    """view-функция для комментрирования поста."""
    # Без отдельной функции не проходят тесты,
    # хотя всё прекрасно работает и так (только с post_view)
    post = get_object_or_404(Post, id=post_id, author__username=username)
    comments = Comment.objects.filter(post=post)
    paginator = Paginator(comments, NUMBER_PAGINATION_PAGES)
    page_number = request.GET.get("page")
...
def profile_unfollow(request, username):
    author = get_object_or_404(User, username=username)
    del_follow = Follow.objects.filter(author=author, user=request.user)
    if author != request.user and del_follow.exists():
        del_follow.delete()
    return redirect("posts:profile", username)
